    ...
    <div className="flex justify-center mt-[5vh] p-2" >
      <Card className="w-full sm:w-[80%] lg:w-[40%]">
        ...
        <CardContent>
          <div className="grid w-full items-center gap-4">
            <div className="flex justify-center">
              <label htmlFor={`${isEdit ? 'avatar' : null}`}>
                <div className="relative flex items-center justify-center">
                  <Avatar className={`w-[150px] h-[150px] ${isEdit && 'opacity-50 cursor-pointer'}`}>
                  ...
                  {isEdit ? uploading ? <LoaderCircle className="w-10 h-10 animate-spin absolute cursor-pointer" /> : <Pencil className="absolute cursor-pointer opacity-80" /> : null}
                </div>
              </label>
              <input disabled={uploading} hidden id='avatar' onChange={handleImageUpload} type='file' accept='image/jpg, image/png, image/jpeg' />
            </div>
            <div className="flex flex-col space-y-1.5">
              <Label htmlFor="name">Username</Label>
              <Input {...register('username', {
                required: true
              })} disabled={!isEdit} id="name" placeholder="Enter your username" />
              {
            ...
            <div className="flex flex-col space-y-1.5">
              ...
              })} disabled={!isEdit} id="name" placeholder="Enter your email" />
              {
        ...
        <CardFooter className="justify-between">
          {
            !isEdit ?
              <Button onClick={() => { setIsEdit(true) }} className="m-auto">Update</Button>
              :
              <>
                ...
                <Button className="flex items-center" onClick={handleSubmit(onSubmit)} >
                  {
                    isLoading && <Loader className="animate-spin h-5 w-5 mr-1" />
                  } Submit
                </Button>
              </>
          }
